...
const roomList = ['KRW-BTC', 'KRW-ETH', 'KRW-XRP']; // WebSocket 방 목록

// ========= 공통 로직 ========= //
// WebSocket 이벤트 핸들러 설정 함수
function handleSocketEvents(socket, userId, roomName) {
    socket.on('open', () => {
        if (DEBUG) console.log(`[Session Test] VU ${userId} connected to room ${roomName}`); // 연결 성공 로그
        ws_sessions.add(1); // 세션 연결 카운터 증가
    });

    socket.on('close', () => {
        if (DEBUG) console.log(`[Session Test] VU ${userId} disconnected from room ${roomName}`); // 연결 종료 로그
        ws_sessions.add(-1); // 세션 종료 메트릭 감소
    });

    socket.on('error', (e) => {
        if (DEBUG) console.log(`[Session Test] VU ${userId} error in room ${roomName}: ${e}`); // 에러 로그
        ws_errors.add(1); // 에러 카운터 증가
    });
}

// WebSocket 연결 테스트 함수
function connectWebSocket(userId, roomName) {
    ...
    const res = ws.connect(url, params, (socket) => {
        handleSocketEvents(socket, userId, roomName); // 이벤트 핸들러 설정
        socket.on('open', () => {
...
export function sessionTestScenario() {
    ...
    const roomName = roomList[__VU % roomList.length]; // 방 이름을 순환적으로 선택
    connectWebSocket(userId, roomName); // WebSocket 연결 테스트 실행
